...
- 6ae3fb6: Re-index a project's issues when who may read it changes, and make the acl sweep come round again.
...
- 3189476: Give indexed issues a real search `acl`, and unblock the three refusals a project-scoped caller met.
...
- bea0aec: Scope the demo seeder's emptiness guard to the workspace.
...
- bccf578: Fill a workspace created with example content: three projects, twenty-five issues across the board,
  labels, components, a version, a milestone and four cycles.

  Written through the module's own services rather than as inserts, so the issues have keys from the
  project counter, ranks, the template's workflow and the status history the cycle report reads. Two
  things it cost a run to learn, both recorded in the file: `StatusCategory` is
  `backlog | todo | in_progress | done | cancelled | triage` and an invented name moves nothing while
  reporting success, and the support project uses the `simple` template because the `support` one
  installs a workspace-level `impact` field marked required — which then refuses every issue in every
...
- 70e9f3f: The tracker's dashboard widgets show their titles, descriptions and settings labels again. They
...
- af5371d: Constrain every workspace-scoped query to its workspace, closing a class of cross-tenant IDOR.
  ...
  **A bare `eq(table.id, input.something)` inside a workspace-scoped transaction is not scoped to
  ...
  - `comments.create` wrote a caller-supplied `parentId` onto the new row and then incremented
    ...
    a comment in another. The parent is now proved to be a comment on _this_ issue in _this_
  ...
  - `defaultAssignees` read `components` by the caller's ids with no workspace predicate, so a
    component whose default assignee is its lead put _that_ lead on an issue created elsewhere.
...
- 8295538: Declare the framework this is built against: `@kernhq/contracts@0.7.0`.
...
- 4e64ef1: Make every migration survive being applied twice.
...
- 5134659: fix: raise @kernhq ranges to what is published
...
- 942e04c: Reach the published framework, and refresh the lockfile that the range edit invalidated.
